...
const StaffsWord = ({ id, level }: Props) => {
  ...
  const [viceWord, setViceWord] = useState(
    '안녕하세요 PRIMITIVE 부회장 20학번 이찬규입니다!',
  );
  const [presidentCharCount, setPresidentCharCount] = useState(
    presidentWord.length,
  );
  ...
  return (
    <div className='p-2'>
      <h3 className='text-xl m-2'>
        😎 회장의 메시지{' '}
        {level === 3 && (
          <small className='text-sm text-gray-500'>수정가능</small>
        )}
      </h3>
